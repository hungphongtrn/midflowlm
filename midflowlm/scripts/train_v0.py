...
def create_dataloaders(config: dict, cache_root: str) -> tuple:
    """Create train and validation dataloaders from cache root.

    Args:
        config: Configuration dictionary
        cache_root: Root directory containing split subdirectories (train/val/test)
    ...
    # Create train dataloader from cache_root/train
    ...
    # Create val dataloader from cache_root/val
...
def main():
    ...
    # Get cache root directory (contains train/val/test subdirectories)
    cache_config = config.get("teacher_cache", {})
    cache_root = cache_config.get("cache_dir", "./cache")

    # Check that split subdirectories exist
    train_cache_dir = Path(cache_root) / "train"
    val_cache_dir = Path(cache_root) / "val"

    if not train_cache_dir.exists():
        logger.error(f"Train cache directory not found: {train_cache_dir}")
        logger.error("Please run scripts/build_teacher_cache.py --split train first")
        sys.exit(1)

    if not val_cache_dir.exists():
        logger.error(f"Val cache directory not found: {val_cache_dir}")
        logger.error("Please run scripts/build_teacher_cache.py --split val first")
        sys.exit(1)

    logger.info(f"Using cache root: {cache_root}")
    logger.info(f"  Train cache: {train_cache_dir}")
    logger.info(f"  Val cache: {val_cache_dir}")

    # Print cache info for train split
    try:
        cache_info = get_cache_info(train_cache_dir)
        logger.info(f"Train cache info: {cache_info}")
    except Exception as e:
        logger.warning(f"Could not load train cache info: {e}")
    ...
    train_dataloader, val_dataloader = create_dataloaders(config, cache_root)
