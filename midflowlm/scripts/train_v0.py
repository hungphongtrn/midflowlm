...
def main():
    ...
    parser.add_argument("--fast-dev-run", action="store_true", help="Run a fast dev loop (1 train, 1 val step)")
    parser.add_argument("--limit-train-batches", type=int, default=None, help="Limit number of training batches")
    parser.add_argument("--limit-val-batches", type=int, default=None, help="Limit number of validation batches")
    ...
    if args.limit_train_batches or args.limit_val_batches:
        ...
        for epoch in range(max_epochs):
            ...
            for batch_idx, batch in enumerate(train_dataloader):
                ...
                if batch_idx % 10 == 0:
                    logger.info(f"  Batch {batch_idx}: loss={metrics['loss']:.4f}, T={metrics['T']}")
            ...
            # Save checkpoint
            checkpoint_dir = Path(config.get("train_loop", {}).get("checkpoint_dir", "./checkpoints"))
