...
warnings.warn(
    "cached_trainer.py is deprecated. "
    "The primary Trainer now uses online_no_cache mode by default. "
    "Import from src.training.trainer instead.",
    DeprecationWarning,
    stacklevel=2,
)
...
class CachedTrainer:
    """DEPRECATED: Raw PyTorch trainer for iterative midblock training with cache support.

    ⚠️ WARNING: This trainer is deprecated. Please use the new `Trainer` class from
    `src.training.trainer` which defaults to online_no_cache mode.
    ...
    - Cache-based training modes (offline_cache, online_write_through_cache)
    ...
    Args:
        ...
        teacher_model: Teacher model for online KL logits (optional)
    """
    ...
    def __init__(
        ...
        warnings.warn(
            "CachedTrainer is deprecated. Use Trainer from src.training.trainer instead, "
            "which defaults to online_no_cache mode. Cache-based training is now optional.",
            DeprecationWarning,
            stacklevel=2,
        )
        self.model = model
