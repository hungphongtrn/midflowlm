"""Online-no-cache trainer for iterative midblock.

This trainer uses token batches only and extracts teacher targets on-the-fly
via model.extract_teacher_targets(), avoiding any teacher-state caching.

Key differences from the base Trainer:
- No teacher_model parameter (uses student's internal frozen Qwen)
- No cache-writing logic or QwenInspector
- Token batches (input_ids, attention_mask) as primary input
- Calls model.extract_teacher_targets() before loss computation
...
class OnlineNoCacheTrainer:
    """Trainer for online-no-cache training with on-the-fly teacher target extraction.
    ...
    - Reuses optimizer/scheduler/checkpoint logic from the base Trainer pattern
