#!/usr/bin/env python3
"""Training script for online-no-cache iterative midblock training with mixed corpus support.

This script trains the Qwen iterative midblock student without any teacher-state
caching. Teacher targets (h_start, h_target, velocity_target, teacher_logits)
are extracted on-the-fly via model.extract_teacher_targets() in one no-grad forward pass.

Supports both TinyStories and mixed corpus datasets via dataset_factory.

Usage:
    python scripts/train_online_no_cache.py --config configs/v0_online_no_cache_mixed_ce_kl.yaml
    python scripts/train_online_no_cache.py --config configs/v0_online_no_cache_mixed_ce_kl.yaml --fast-dev-run
    python scripts/train_online_no_cache.py --config configs/v0_online_no_cache_mixed_ce_kl.yaml --limit-train-batches 10
"""
...
def main():
    ...
    logger.info("Creating OnlineNoCacheTrainer...")
